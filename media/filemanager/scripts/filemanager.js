...
if(django_view != ""){
	...
	var treeConnector = '/media/filemanager/scripts/jquery.filetree/connectors/jqueryFileTree.' + lang;
	var fileConnector = '/media/filemanager/connectors/' + lang + '/filemanager.' + lang;
}
...
var setUploader = function(path){
	...
	$('#uploader h1').text('Текущая папка: ' + path);

	$('#newfolder').unbind().click(function(){
		// var foldername = prompt('Enter the name of the new folder:', 'My Folder');
		var foldername = 'Новая папка';
		var msg = 'Введите название новой папки: <input id="fname" name="fname" type="text" value="' + foldername + '" />';
		...
		$.prompt(msg, {
			...
			buttons: { 'Создать папку': 1, 'Cancel': 0 }
		});		
	});	
}
...
var bindToolbar = function(data){
	...
	$('#fileinfo').find('button#delete').click(function(){
		if(deleteItem(data)) $('#fileinfo').html('<h1>Выберете файл в лефой части.</h1>');
	});
...
var selectItem = function(data){
	if(window.opener){
		...
		$.prompt('Функция выбрать доступна только при использовании с тектовым редактором.');
	}
}
...
var renameItem = function(data){
	var finalName = '';
	var msg = 'Ведите новое название файла: <input id="rname" name="rname" type="text" value="' + data['Filename'] + '" />';
	...
	$.prompt(msg, {
		callback: getNewName,
		buttons: { 'Переименовать': 1, 'Cancel': 0 }
	});
...
var getFileInfo = function(file){
	...
	template += '<button id="select" name="select" type="button" value="Select">Выбрать</button>';
	template += '<button id="download" name="download" type="button" value="Download">Скачать</button>';
	template += '<button id="rename" name="rename" type="button" value="Rename">Переименовать</button>';
	template += '<button id="delete" name="delete" type="button" value="Delete">Удалить</button>';
	template += '</form>';
	...
	$.getJSON(fileConnector + '?mode=getinfo&path=' + file, function(data){
		if(data['Code'] == 0){
			...
			if(data['Properties']['Width'] && data['Properties']['Width'] != '') properties += '<dt>Размер</dt><dd>' + data['Properties']['Width'] + 'x' + data['Properties']['Height'] + '</dd>';
			if(data['Properties']['Date Created'] && data['Properties']['Date Created'] != '') properties += '<dt>Создан</dt><dd>' + data['Properties']['Date Created'] + '</dd>';
			if(data['Properties']['Date Modified'] && data['Properties']['Date Modified'] != '') properties += '<dt>Изменен</dt><dd>' + data['Properties']['Date Modified'] + '</dd>';
			if(data['Properties']['Size'] && data['Properties']['Size'] != '') properties += '<dt>Размер</dt><dd>' + formatBytes(data['Properties']['Size']) + '</dd>';
...
var getFolderInfo = function(path){
	...
	$('#fileinfo').html('<img id="activity" src="/media/filemanager/images/wait30trans.gif" width="30" height="30" />');
	...
	$.getJSON(fileConnector + '?path=' + path + '&mode=getfolder&showThumbs=' + showThumbs, function(data){		
		...
		if(data){		
			if($('#fileinfo').data('view') == 'grid'){
				...
				result += '<thead><tr><th class="headerSortDown"><span>Название</span></th><th><span>Пропорции</span></th><th><span>Размер</span></th><th><span>Изменен</span></th></tr></thead>';
				result += '<tbody>';

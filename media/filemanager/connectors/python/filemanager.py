...
try:
    from settings_local import UPLOAD_DIR
except ImportError:
...
class Filemanager:
    ...
    def getinfo(self, path=None, getsize=true, req=None):
        ...
        thefile = {
            ...
            'File Type' : '',
            'Preview' : path if split_path(path)[-1] else '/media/filemanager/images/fileicons/_Open.png',
            'Path' : path,
            'Error' : '',
            'Code' : 0,
            'Properties' : {
        ...
        if split_path(path)[-1]=='/':
            ...
            if ext in imagetypes:
                ...
                thefile['Preview'] = previewPath if path_exists('../../' + previewPath) else '/media/filemanager/images/fileicons/default.png'
...
myFilemanager = Filemanager(fileroot=PROJECT_DIR + UPLOAD_DIR) #modify fileroot as a needed
